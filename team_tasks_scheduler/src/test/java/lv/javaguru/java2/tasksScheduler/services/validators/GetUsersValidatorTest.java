...
public class GetUsersValidatorTest {
    ...
    public void shouldSuccessWhenDescOrderDirection() {
        GetUsersRequest request = new GetUsersRequest("test", "test@test.com",
                new Ordering("username", "DESCENDING"));
        List<CoreError> errors = validator.validate(request, MenuType.START);
        assertEquals(errors.size(), 0);
    }
    ...
    @Test
    public void shouldReturnErrorWhenPageSizeButPageNumberIsNotProvided() {
        GetUsersRequest request = new GetUsersRequest("test", "test@test.com",
                new Paging(null, 1));
        List<CoreError> errors = validator.validate(request, MenuType.START);
        assertEquals(errors.size(), 1);
    ...
    @Test
    public void shouldReturnErrorWhenPageNumberButPageSizeIsNotProvided() {
        GetUsersRequest request = new GetUsersRequest("test", "test@test.com",
                new Paging(1, null));
        List<CoreError> errors = validator.validate(request, MenuType.START);
        assertEquals(errors.size(), 1);
