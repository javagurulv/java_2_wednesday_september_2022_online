...
class VehicleAvailabilityServiceValidatorTest {
    ...
    void testGoodScenario() {
        ...
        String rentStartDate = LocalDate.now().plusDays(1).format(formatter).toString();
        String rentEndDate = LocalDate.now().plusDays(5).format(formatter).toString();
    ...
    @Test
    void testValidateStartDateIsPresentIsEmpty() {
        DateTimeFormatter formatter =DateTimeFormatter.ofPattern("dd/MM/yyyy");
        String rentEndDate = LocalDate.now().plusDays(5).format(formatter).toString();
   ...
   @Test
    void testValidateStartDateNotInPast() {
       DateTimeFormatter formatter =DateTimeFormatter.ofPattern("dd/MM/yyyy");
       String rentStartDate = LocalDate.now().minusDays(1).format(formatter).toString();
       String rentEndDate = LocalDate.now().plusDays(5).format(formatter).toString();
       GeneralRentVehicleRequest request = GeneralRentVehicleRequest.builder()
               .rentStartDate(rentStartDate).rentEndDate(rentEndDate).build();
       List<CoreError> errors = validator.execute(request);
   ...
   @Test
    void testValidateEndDateNotInPast() {
       DateTimeFormatter formatter =DateTimeFormatter.ofPattern("dd/MM/yyyy");
       String rentStartDate = LocalDate.now().plusDays(1).format(formatter).toString();
       String rentEndDate = LocalDate.now().minusDays(5).format(formatter).toString();
       GeneralRentVehicleRequest request = GeneralRentVehicleRequest.builder()
               .rentStartDate(rentStartDate).rentEndDate(rentEndDate).build();
       List<CoreError> errors = validator.execute(request);
   ...
   @Test
    void testValideteStartDateIsNotToFarReturnError() {
       DateTimeFormatter formatter =DateTimeFormatter.ofPattern("dd/MM/yyyy");
       String rentStartDate = LocalDate.now().plusYears(3).format(formatter).toString();
       String rentEndDate = LocalDate.now().plusYears(4).format(formatter).toString();
